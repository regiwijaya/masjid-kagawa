...
export default function AdminPosts() {
  ...
  const togglePublish = async (item) => {
    await http.put(`/api/posts/${item.id}`, { isPublished: !item.isPublished }, { headers });
    fetchItems();
  };

  const toggleFeatured = async (item) => {
    await http.put(`/api/posts/${item.id}`, { isFeatured: !item.isFeatured }, { headers });
    fetchItems();
  };
        ...
        <div className="admin-toolbar">
          <div>
            ...
            <div className="admin-muted">
              CMS Artikel (Search, Filter, Editor)
            </div>
          </div>
        </div>

        {(err || info) && (
          <div className={`admin-posts-notice ${err ? "admin-posts-notice--error" : "admin-posts-notice--success"}`}>
            {err || info}
          </div>
        )}
              ...
              {filteredItems.map((item) => (
                <div key={item.id} className="admin-posts-item">
                  ...
                  <div className="admin-posts-item__actions">
                    <button onClick={() => startEdit(item)} className="admin-btn">Edit</button>
                    ...
                    <button onClick={() => handleDelete(item.id)} className="admin-btn admin-btn-danger">
                      Hapus
                    </button>
                  </div>
